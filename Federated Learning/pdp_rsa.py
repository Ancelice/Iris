...
df = pd.read_excel("./pdp/Asia/" + filename[0], engine="openpyxl")
train_rsa = df['probe_dst_prefix']
tmp1 = df['reply_src_addr']
train_x = []
train_y = []
for z in range(len(tmp1)):
    train_x.append(int(ipaddress.IPv4Address(df.iat[z, 1])))
    train_y.append(int(ipaddress.IPv4Address(df.iat[z, 3])))
...
for i in range(len(filename) - 1):
    ...
    # train / Africa
    df = pd.read_excel("./pdp/Africa/" + filename[i], engine="openpyxl")
    train_rsa = df['probe_dst_prefix']
    tmp1 = df['reply_src_addr']
    AFtrain_x = []
    AFtrain_y = []
    for a in range(len(tmp1)):
        AFtrain_x.append(int(ipaddress.IPv4Address(df.iat[a, 1])))
        AFtrain_y.append(int(ipaddress.IPv4Address(df.iat[a, 3])))
    ...
    # train / Asia
    df = pd.read_excel("./pdp/Asia/" + filename[i], engine="openpyxl")
    train_rsa = df['probe_dst_prefix']
    tmp1 = df['reply_src_addr']
    AStrain_x = []
    AStrain_y = []
    for b in range(len(tmp1)):
        AStrain_x.append(int(ipaddress.IPv4Address(df.iat[b, 1])))
        AStrain_y.append(int(ipaddress.IPv4Address(df.iat[b, 3])))
    ...
    for c in range(len(tmp1)):
        EUtrain_x.append(int(ipaddress.IPv4Address(df.iat[c, 1])))
        EUtrain_y.append(int(ipaddress.IPv4Address(df.iat[c, 3])))
    ...
    # train / NA
    df = pd.read_excel("./pdp/NA/" + filename[i], engine="openpyxl")
    train_rsa = df['probe_dst_prefix']
    tmp1 = df['reply_src_addr']
    NAtrain_x = []
    NAtrain_y = []
    for d in range(len(tmp1)):
        NAtrain_x.append(int(ipaddress.IPv4Address(df.iat[d, 1])))
        NAtrain_y.append(int(ipaddress.IPv4Address(df.iat[d, 3])))
    ...
    # train / Oceania
    df = pd.read_excel("./pdp/Oceania/" + filename[i], engine="openpyxl")
    train_rsa = df['probe_dst_prefix']
    tmp1 = df['reply_src_addr']
    OCtrain_x = []
    OCtrain_y = []
    for e in range(len(tmp1)):
        OCtrain_x.append(int(ipaddress.IPv4Address(df.iat[e, 1])))
        OCtrain_y.append(int(ipaddress.IPv4Address(df.iat[e, 3])))
    ...
    # train / SA
    df = pd.read_excel("./pdp/SA/" + filename[i], engine="openpyxl")
    train_rsa = df['probe_dst_prefix']
    tmp1 = df['reply_src_addr']
    SAtrain_x = []
    SAtrain_y = []
    for f in range(len(tmp1)):
        SAtrain_x.append(int(ipaddress.IPv4Address(df.iat[f, 1])))
        SAtrain_y.append(int(ipaddress.IPv4Address(df.iat[f, 3])))
    ...
    # test / Africa
    df2 = pd.read_excel("./pdp/Africa/" + filename[i + 1], engine="openpyxl")
    test_rsa = df2['reply_src_addr']
    tmp2 = df2['reply_src_addr']
    AFtest_x = []
    AFtest_y = []
    editName = filename[i + 1]
    editName = editName.replace('./pdp/Africa/', '')
    ...
    for aa in range(len(tmp2)):
        AFtest_x.append(int(ipaddress.IPv4Address(df2.iat[aa, 1])))
        AFtest_y.append(int(ipaddress.IPv4Address(df2.iat[aa, 3])))
        AFTIME.append(editName)
        AFOBS.append(test_rsa[aa])
    ...
    # test / Asia
    df2 = pd.read_excel("./pdp/Asia/" + filename[i + 1], engine="openpyxl")
    test_rsa = df2['reply_src_addr']
    tmp2 = df2['reply_src_addr']
    AStest_x = []
    AStest_y = []
    editName = filename[i + 1]
    editName = editName.replace('./pdp/Asia/', '')
    editName = editName.replace('log_', '')
    editName = editName.replace('.xlsx', '')
    editName = editName[:4] + "/" + editName[4:]
    editName = editName[:7] + "/" + editName[7:]
    for bb in range(len(tmp2)):
        AStest_x.append(int(ipaddress.IPv4Address(df2.iat[bb, 1])))
        AStest_y.append(int(ipaddress.IPv4Address(df2.iat[bb, 3])))
        ASTIME.append(editName)
        ASOBS.append(test_rsa[bb])
    ...
    # test / Europe
    df2 = pd.read_excel("./pdp/Europe/" + filename[i + 1], engine="openpyxl")
    test_rsa = df2['reply_src_addr']
    tmp2 = df2['reply_src_addr']
    EUtest_x = []
    EUtest_y = []
    editName = filename[i + 1]
    editName = editName.replace('./pdp/Europe/', '')
    editName = editName.replace('log_', '')
    editName = editName.replace('.xlsx', '')
    editName = editName[:4] + "/" + editName[4:]
    editName = editName[:7] + "/" + editName[7:]
    for cc in range(len(tmp2)):
        EUtest_x.append(int(ipaddress.IPv4Address(df2.iat[cc, 1])))
        EUtest_y.append(int(ipaddress.IPv4Address(df2.iat[cc, 3])))
        EUTIME.append(editName)
        EUOBS.append(test_rsa[cc])
    ...
    # test / NA
    df2 = pd.read_excel("./pdp/NA/" + filename[i + 1], engine="openpyxl")
    test_rsa = df2['reply_src_addr']
    tmp2 = df2['reply_src_addr']
    NAtest_x = []
    NAtest_y = []
    editName = filename[i + 1]
    editName = editName.replace('./pdp/NA/', '')
    editName = editName.replace('log_', '')
    editName = editName.replace('.xlsx', '')
    editName = editName[:4] + "/" + editName[4:]
    editName = editName[:7] + "/" + editName[7:]
    for dd in range(len(tmp2)):
        NAtest_x.append(int(ipaddress.IPv4Address(df2.iat[dd, 1])))
        NAtest_y.append(int(ipaddress.IPv4Address(df2.iat[dd, 3])))
        NATIME.append(editName)
        NAOBS.append(test_rsa[dd])
    ...
    # test / Oceania
    df2 = pd.read_excel("./pdp/Oceania/" + filename[i + 1], engine="openpyxl")
    test_rsa = df2['reply_src_addr']
    tmp2 = df2['reply_src_addr']
    OCtest_x = []
    OCtest_y = []
    editName = filename[i + 1]
    editName = editName.replace('./pdp/Oceania/', '')
    editName = editName.replace('log_', '')
    editName = editName.replace('.xlsx', '')
    editName = editName[:4] + "/" + editName[4:]
    editName = editName[:7] + "/" + editName[7:]
    for ee in range(len(tmp2)):
        OCtest_x.append(int(ipaddress.IPv4Address(df2.iat[ee, 1])))
        OCtest_y.append(int(ipaddress.IPv4Address(df2.iat[ee, 3])))
        OCTIME.append(editName)
        OCOBS.append(test_rsa[ee])
    ...
    # test / SA
    df2 = pd.read_excel("./pdp/SA/" + filename[i + 1], engine="openpyxl")
    test_rsa = df2['reply_src_addr']
    tmp2 = df2['reply_src_addr']
    SAtest_x = []
    SAtest_y = []
    editName = filename[i + 1]
    editName = editName.replace('./pdp/SA', '')
    editName = editName.replace('log_', '')
    editName = editName.replace('.xlsx', '')
    editName = editName[:4] + "/" + editName[4:]
    editName = editName[:7] + "/" + editName[7:]
    for ff in range(len(tmp2)):
        SAtest_x.append(int(ipaddress.IPv4Address(df2.iat[ff, 1])))
        SAtest_y.append(int(ipaddress.IPv4Address(df2.iat[ff, 3])))
        SATIME.append(editName)
        SAOBS.append(test_rsa[ff])
    ...
    for g in range(len(tmp3)):
        MAStest_x.append(int(ipaddress.IPv4Address(df3.iat[g, 1])))
        MAStest_y.append(int(ipaddress.IPv4Address(df3.iat[g, 3])))
        MASTIME.append(editName)
        MASOBS.append(test_rsa[g])
    ...
    for aaa in range(len(AFpred)):
        AFPRED.append(str(ipaddress.IPv4Address(int(AFpred[aaa]))))
        AFMSE.append(LOSS)
        AFMAE.append(MAE)
    ...
    for bbb in range(len(ASpred)):
        ASPRED.append(str(ipaddress.IPv4Address(int(ASpred[bbb]))))
        ASMSE.append(LOSS)
        ASMAE.append(MAE)
    ...
    for ccc in range(len(EUpred)):
        EUPRED.append(str(ipaddress.IPv4Address(int(EUpred[ccc]))))
        EUMSE.append(LOSS)
        EUMAE.append(MAE)
    ...
    for ddd in range(len(NApred)):
        NAPRED.append(str(ipaddress.IPv4Address(int(NApred[ddd]))))
        NAMSE.append(LOSS)
        NAMAE.append(MAE)
    ...
    for eee in range(len(OCpred)):
        OCPRED.append(str(ipaddress.IPv4Address(int(OCpred[eee]))))
        OCMSE.append(LOSS)
        OCMAE.append(MAE)
    ...
    for fff in range(len(SApred)):
        SAPRED.append(str(ipaddress.IPv4Address(int(SApred[fff]))))
        SAMSE.append(LOSS)
        SAMAE.append(MAE)
    ...
    for gg in range(len(MASpred)):
        MASPRED.append(str(ipaddress.IPv4Address(int(MASpred[gg]))))
        MASMSE.append(LOSS)
        MASMAE.append(MAE)
    ...
    savename = './result/pdp_rsa/Africa/' +  tmpName + '.xlsx'
    ...
    tmpName = editName.replace('log_', '')
    tmpName = tmpName.replace('/', '_')
    savename = './result/pdp_rsa/Asia/' +  tmpName + '.xlsx'
    ...
    tmpName = editName.replace('log_', '')
    tmpName = tmpName.replace('/', '_')
    savename = './result/pdp_rsa/Europe/' +  tmpName + '.xlsx'
    ...
    tmpName = editName.replace('log_', '')
    tmpName = tmpName.replace('/', '_')
    savename = './result/pdp_rsa/NA/' +  tmpName + '.xlsx'
    ...
    tmpName = editName.replace('log_', '')
    tmpName = tmpName.replace('/', '_')
    savename = './result/pdp_rsa/Oceania/' +  tmpName + '.xlsx'
    ...
    tmpName = editName.replace('log_', '')
    tmpName = tmpName.replace('/', '_')
    savename = './result/pdp_rsa/SA/' +  tmpName + '.xlsx'
    ...
    for yy in range(len(MASPRED)):
        ...
        ws['E' + str(h) ] = MASPRED[yy]
        h = h + 1
    tmpName = filename[i + 1]
    tmpName = editName.replace('log_', '')
    tmpName = tmpName.replace('/', '_')
    savename = './result/pdp_rsa/Master/' +  tmpName + '.xlsx'
