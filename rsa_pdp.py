...
tmp1 = df['probe_dst_prefix']
train_x = []
train_y = []
for m in range(len(tmp1)):
    train_x.append(int(ipaddress.IPv4Address(df.iat[m, 3])))
    train_y.append(int(ipaddress.IPv4Address(df.iat[m, 1])))
...
for i in range(len(filename) - 2):
    ...
    print()
    print()
    print('----- Epoch : ' + str(i + 1) + ' / ' + str(len(filename) - 2) + ' -----')
    ...
    for j in range(len(tmp1)):
        train_x.append(int(ipaddress.IPv4Address(df.iat[j, 3])))
        train_y.append(int(ipaddress.IPv4Address(df.iat[j, 1])))
    ...
    for k in range(len(tmp2)):
        test_x.append(int(ipaddress.IPv4Address(df2.iat[k, 3])))
        test_y.append(int(ipaddress.IPv4Address(df2.iat[k, 1])))
    ...
    savename = './predict/rsa_pdp/' +  tmpName + '.xlsx'
    print('Save : ' + savename)
